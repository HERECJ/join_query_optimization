...
class Query_Init():
    ...
    def get_Actions(self, sql, masks, relations_id, schema, indices,column_dict):
        ...
        for r in relations:
            ...
            join_table_name = r_split[-1]  # if no AS, then the final
            action_space.append(Relation(
                r, join_table_name, masks[join_table_name], schema[join_table_name], relations_id[join_table_name], indices,column_dict))
...
class Query(object):
    ...
    def __init__(self, left, right):
        ...
        for c in right.columns:
            ...
            if c.split('.')[1] in tmpcolumns:
                ...
                new_column = rname+"."+str(c.split('.')[0].split("&")[0]+c.split('.')[0].split("&")[-1])+"$"+c.split('.')[1]
    ...
    def _toSql(self, level):
        ...
        if type(self.left) is Relation and type(self.right) is Relation:
            # subsql_left = self.left.sql_name
            subsql_left = self.left.clause_to_sql()

            # subsql_right = self.right.sql_name
    ...
    def to_tree_structure(self):
        ...
        if len(self.join_condition) is not 0:
            ...
            if self.join_condition_id[0] in  self.left.embed and self.join_condition_id[1] in  self.right.embed:
                ...
                tree.l_column_embed .append(self.right.embed[self.join_condition_id[0]])
                tree.r_column_embed .append(self.left.embed[self.join_condition_id[1]])
                if type(self.left) is Relation and type(self.right) is Relation:
                    tree.l_name =  self.right.name
                    tree.l_table_id = self.right.id
                    tree.l_table_embed = [self.right.embed[value] for key, value in enumerate(self.right.embed)]

                    tree.r_name =  self.left.name
                    tree.r_table_id = self.left.id
                    tree.r_table_embed = [self.left.embed[value] for key, value in enumerate(self.left.embed)]

                elif type(self.left) is Relation and type(self.right) is Query:
                    tree.l_name = self.right.to_tree_structure()
                    tree.l_table_id = None
                    tree.l_table_embed = None
                    ...
                    tree.r_name = self.left.to_tree_structure()
                    tree.r_table_id = None
                    tree.r_table_embed = None
            ...
            if len(self.join_condition) > 2:
                for i in range(2, len(self.join_condition), 2):
                    ...
                    elif self.join_condition_id[i + 1] in self.left.embed and self.join_condition_id[i] in self.right.embed:
                        tree.l_column_embed .append(self.right.embed[self.join_condition_id[i]])
                        tree.r_column_embed .append(self.left.embed[self.join_condition_id[i + 1]])

            # print(tree.l_column)
            # print(tree.r_column)
            # print(tree.l_column_id)
            # print(tree.r_column_id)
            # print(tree.l_column_embed)
            # print(tree.r_column_embed)
