...
from agent.policy import Tree_Cus
from env.masking_env import Tree_Env
import argparse
import torch
import numpy as np
import os
...
def main(config, log_dir):
    ...
    env = Tree_Env({"file_name":file_name + '_train.txt', 'process': 'train', "_tree_flag_": True})
    eval_env = Tree_Env({"file_name":file_name + '_test.txt', 'process':'eval', "_tree_flag_": True})
    ...
    agent_a = Tree_Cus(config)

    div = torch.device(device)

    # obs['db'] = torch.zeros(config['emb_dim'], device=div)
    obs['db'] = None
    first_ep = True
    loss_0 = 0.0
    epi = 0
    for i in range(config['episodes']):
        ...
        if not done:
            obs['db'] = agent_a.policy_net.tree_encoding(obs['query_lst'])
        ...
        if done is True:
            ...
            if epi % config['Test'] ==0:
                ...
                loss_0 = 0.0
                with torch.no_grad():
                    avg_rows, avg_cost = eval(agent_a, epi, eval_env, log_dir)
...
def eval(agent_a, episodes, env, log_dir):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-b', '--BATCH_SIZE', default=32, type=int, help='BATCH_SIZE for training')
    parser.add_argument('-f', '--fold', default=2, type=int, help='data file')
    parser.add_argument('-ed', '--emb_dim', default=16, type=int)
    parser.add_argument('-e', '--episodes', default=20000, type=int)
    ...
    parser.add_argument('--rel_pretrain', default=True)
