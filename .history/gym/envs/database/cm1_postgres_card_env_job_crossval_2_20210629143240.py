...
class CM1PostgresCardJob2(gym.Env):
  ...
  def __init__(self):
    ...
    # self.sql_query = list(open('~//PycharmProjects/mt-join-queryoptimization-with-drl/agents/queries/crossval_sens/job_queries_simple_crossval_2_test.txt'))
    self.sql_query = list(open('/data0/chenjin/mt-join-query-optimization-with-drl/agents/queries/crossval_sens/job_queries_simple_crossval_2_test.txt'))
    self.sql_query_num = 0


    try:
        # conn = psycopg2.connect(host="localhost", database="imdbload", user="postgres", password="admin")
        conn = psycopg2.connect(host="localhost", database="exp", user="imdb", password="ych19960128", host='127.0.0.1',  port='5433')
    except:
  ...
  def reset(self):
    ...
    #sql_query = random.choice(list(open('~//PycharmProjects/mt-join-queryoptimization-with-drl/agents/queries/crossval_sens/job_queries_simple_crossval_2_train.txt'))).replace(";", "")

    sql_query = random.choice(list(open('/data0/chenjin/mt-join-query-optimization-with-drl/agents/queries/crossval_sens/job_queries_simple_crossval_2_train.txt'))).replace(";","")
