...
class CustomModel(TorchModelV2):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        super(CustomModel).__init__(obs_space, action_space, num_outputs, model_config, name)
        nn.Module.__init__()
        # self.fcnet = nn.Linear(3024, num_outputs)
        combined_space = Box(0, 1, shape=(3024,), dtype=np.float32)
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        output_dict,state = self.fcnet(input_dict)
        # inf_mask = torch.max(torch.log(action_mask), tf.float32.min)
        inf_mask = torch.max(torch.log(action_mask), torch.tensor(-np.inf))
